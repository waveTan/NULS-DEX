        ...
        <div class="mt_20">
          <el-table :data="entrustData">
            ...
            <el-table-column label="成交率" width="100" align="center">
              <template slot-scope="scope">
                <span>{{scope.row.turnover*100}}%</span>
              </template>
            </el-table-column>
            <el-table-column label="操作" min-width="60" align="center">
              <template slot-scope="scope">
                <el-link type="primary" @click="revoke(scope.row)">撤销</el-link>
              </template>
            </el-table-column>
          </el-table>
        </div>
      ...
      <el-tab-pane label="历史成交" name="historyDeal" class="history_deal">
        <div class="mt_20">
          <div class="select">
            ...
            <el-button type="primary" icon="el-icon-search" class="select_input" @click="search(0)">搜索
            </el-button>
          </div>
          <el-table :data="historicalData">
            <el-table-column prop="hashs" label="交易HASH" width="180" align="center">
            </el-table-column>
            <el-table-column prop="time" label="时间" width="180" align="center">
            </el-table-column>
            <el-table-column prop="tradingName" label="交易对" width="180" align="center">
            </el-table-column>
            ...
            <el-table-column prop="prices" label="单价" width="180" align="center">
            </el-table-column>
            ...
            <el-table-column label="手续费" min-width="140" align="center">
              <template slot-scope="scope">
                <span>{{scope.row.type === 1 ? scope.row.buyFee:scope.row.sellFee}}</span>
              </template>
            </el-table-column>
          </el-table>
        </div>
      ...
      <el-tab-pane label="历史委托" name="historyEntrust" class="history_entrust">
        <div class="mt_20">
          <div class="select">
            ...
            <el-button type="primary" icon="el-icon-search" class="select_input" @click="search(1)">搜索</el-button>
          </div>
...
<script>
  ...
  import {divisionDecimals, getLocalTime, superLong, Division, connect} from '@/api/util.js'
    ...
    methods: {
      ...
      async getEntrustList(address, pageIndex, pageSize) {
        ...
        if (!entrustListRes.success) {
          this.$message({
            message: '获取用户当前委托列表错误:' + JSON.stringify(entrustListRes.data),
            type: 'error',
            duration: 3000
          });
        }
        ...
        for (let item of entrustListRes.result.list) {
          ...
          item.quantity = parseFloat(Number(divisionDecimals(item.baseDealAmount, item.baseDecimal)).toFixed(5));
          item.total = parseFloat(Number(divisionDecimals(item.quoteAmount, item.quoteDecimal)).toFixed(5));
          item.totalTurnover = parseFloat(Number(divisionDecimals(item.quoteDealAmount, item.quoteDecimal)).toFixed(5));
          item.turnover = parseFloat(Division(Number(item.totalTurnover), Number(item.total)).toFixed(2));
        }
      ...
      async getHistoricalList(address, pageIndex, pageSize, beginTime, endTime) {
        ...
        for (let item of entrustListRes.result.list) {
          ...
          item.prices = parseFloat(Number(divisionDecimals(item.price, item.quoteDecimal)).toFixed(5));
          item.number = parseFloat(Number(divisionDecimals(item.baseAmount, item.baseDecimal)).toFixed(5));
          item.totalTurnover = parseFloat(Number(divisionDecimals(item.quoteAmount, item.quoteDecimal)).toFixed(5));
        }
